...
app.controller('myCtrl', function ($scope, $http, $interval, $timeout, $compile) {
    ...
    $scope.getPrescriptions = function (appt_id) {
        ...
        $http({
            method: 'POST',
            url: 'http://192.168.1.150:9090/get_Doctor_Prescriptions',
            headers: {
    ...
    var openLightboxModal = function (presArray, pageno) {
        ...
        if (presArray.image_data_final.length === 1) {
            $scope.isNext = false;
            $scope.isPrev = false;
        } else if (presArray.image_data_final.length > 1) {
            $scope.isNext = true;
            $scope.isPrev = true;
        }
        ...
        switch (true) {
            ...
            case pageno >= presArray.image_data_final.length - 1:
                pageno = presArray.image_data_final.length - 1;
                $scope.isNext = false;
                break;
        ...
        var images = presArray.image_data_final;
        console.log(images.length);
        var presImg = $('#lightgallery').empty();
        var img = images[pageno];
